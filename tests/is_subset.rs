...
#[test]
fn arrays() {
    let sup = RoaringBitmap::from_iter((0..3000).chain(100000..103000));
    let sub = RoaringBitmap::from_iter((0..2000).chain(100000..102000));
    assert_eq!(sub.is_subset(&sup), true);
}

#[test]
fn bitmaps_not() {
    let sup = RoaringBitmap::from_iter((0..6000).chain(1000000..1006000).chain(2000000..2010000));
    let sub = RoaringBitmap::from_iter((100000..106000).chain(1100000..1106000));
    assert_eq!(sub.is_subset(&sup), false);
}
