...
fn remove_range() {
    ...
    for (i, &a) in ranges.iter().enumerate() {
        for &b in &ranges[i..] {
            let mut bitmap = (0..=65536).collect::<Roaring64>();
            assert_eq!(bitmap.remove_range(a..b), (b - a));
            assert_eq!(bitmap, ((0..a).chain(b..=65536)).collect::<Roaring64>());
        }
    }
}
...
#[test]
fn to_bitmap() {
    let bitmap = (0..5000).collect::<Roaring64>();
...
#[test]
fn to_array() {
    let mut bitmap = (0..5000).collect::<Roaring64>();
    for i in 3000..5000 {
        bitmap.remove(i);
    }
