...
fn rank_roaring_bitmaps() {
    // A bitmap with two roaring bitmaps.
    ...
    let bitmap = Roaring64::from_sorted_iter(BITMAP_MAX - 1000..BITMAP_MAX + 5000).unwrap();

    // start of bitmap
    assert_eq!(bitmap.rank(0), 0);

    // low boundary
    assert_eq!(bitmap.rank(BITMAP_MAX - 1002), 0);
    assert_eq!(bitmap.rank(BITMAP_MAX - 1001), 0);
    assert_eq!(bitmap.rank(BITMAP_MAX - 1000), 1);

    // middle range (spans two roaring bitmaps)
    assert_eq!(bitmap.rank(BITMAP_MAX - 1), 1000);
    assert_eq!(bitmap.rank(BITMAP_MAX), 1001);
    assert_eq!(bitmap.rank(BITMAP_MAX + 1), 1002);

    // high boundary
    assert_eq!(bitmap.rank(BITMAP_MAX + 4998), 5999);
    assert_eq!(bitmap.rank(BITMAP_MAX + 4999), 6000);
    assert_eq!(bitmap.rank(BITMAP_MAX + 5000), 6000);

    // end of bitmap
    assert_eq!(bitmap.rank(u64::MAX), 6000);
}
