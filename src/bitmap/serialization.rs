...
impl RoaringBitmap {
    ...
    /// [RoaringBitmap::deserialize_from_nonvalidating]
    ///
    /// [format]: https://github.com/RoaringBitmap/RoaringFormatSpec
    ///
    /// # Examples
    ...
    /// let mut bytes = vec![];
    /// rb1.serialize_into(&mut bytes).unwrap();
    /// let rb2 = RoaringBitmap::deserialize_from_nonvalidating(&bytes[..]).unwrap();
    ///
    /// assert_eq!(rb1, rb2);
    /// ```
    pub fn deserialize_from_nonvalidating<R: io::Read>(reader: R) -> io::Result<RoaringBitmap> {
