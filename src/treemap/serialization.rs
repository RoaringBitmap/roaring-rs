...
impl RoaringTreemap {
    ...
    /// let mut bytes = vec![];
    /// rb1.serialize_into(&mut bytes).unwrap();
    /// let rb2 = RoaringTreemap::deserialize_from_nonvalidating(&bytes[..]).unwrap();
    ///
    /// assert_eq!(rb1, rb2);
    /// ```
    pub fn deserialize_from_nonvalidating<R: io::Read>(reader: R) -> io::Result<Self> {
        RoaringTreemap::deserialize_from_impl(reader, |reader| {
            RoaringBitmap::deserialize_from_nonvalidating(reader)
        })
    }
