...
fn main() {
    ...
    let benches_dir = PathBuf::from(env::var("CARGO_MANIFEST_DIR").unwrap()).join("benches");
    let mut manifest_paths_file = File::create(benches_dir.join("datasets_paths.rs")).unwrap();
    ...
    for dataset in &[DATASET_WIKILEAKS_NOQUOTES_SRT] {
        ...
        let bytes = download_dataset(url).unwrap();
        unzip_in_folder(bytes, &out_path).unwrap();
